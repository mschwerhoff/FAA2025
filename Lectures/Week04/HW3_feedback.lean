...
/-! # GRADING FEEDBACK
   ...
   Excellent work! Your thoughtful questions show great learning curiosity.

   Re your Question 1 (len vs len.eq_def):
   - `len` refers directly to the function definition
   - `len.eq_def` is an equation lemma auto-generated by Lean that explicitly unfolds the
     definition in equational form
   - In most cases they work the same, but `len.eq_def` can be more explicit in certain
     contexts where you want the definitional equality made visible

   Re your Question 2 (partial match simplification):
   - To resolve just the match block without closing the goal, you can use `unfold` to
     expand definitions partially
   - Or use `show [new_goal]` to restate the goal in an equivalent form
   - `conv` mode also allows targeted rewriting: `conv => lhs; unfold S`
   - However, `simp` is usually smart enough to handle this automatically, as you found!
-/
